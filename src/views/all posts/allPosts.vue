    ...
    <section style="min-height: 700px" class="d-flex">
      ...
      <div class="form-articles col-12 col-lg-10">
        <div class="d-flex justify-content-between">
          ...
          <div>
            <button
              @click="yourArticles = false"
              class="btn"
              :class="{ 'btn-secondary': !yourArticles }"
            >
              all articles
            </button>
            <button
              @click="(yourArticles = true), (currentPage = 1)"
              class="btn"
              :class="{ 'btn-secondary': yourArticles }"
            >
              your article
        ...
        <div
          class="table-responsive my-4 overflw-x-auto position-relative d-flex flex-column align-items-center"
        >
          <div
            v-if="isLoading"
          ...
          >
            <button
              ...
              class="previousPageBtn btn btn-outline-secondary btn-white"
            >
              &lt;
            </button>
            ...
            <template v-for="page in totalPages" :key="page">
              ...
              <button
                ...
                class="pageNumberBtn btn btn-outline-secondary"
            ...
            <button
              ...
              class="nextPageBtn btn btn-outline-secondary btn-white"
            >
              &gt;
            </button>
          </div>
...
export default {
    ...
    axios
      ...
      .then((response) => {
        ...
        const addedArticles = myArticles.map((element) => {
          const { title, author, tagList, description, createdAt, slug } =
            element;
  ...
  computed: {
    ...
    totalArticlesByAthor() {
      return this.shownArticles.filter(
        (article) => article.author == this.username
      );
    },

    displayedArticles() {
      if (this.yourArticles) {
        return this.totalArticlesByAthor.slice(
          (this.currentPage - 1) * 8,
          this.currentPage * 8
        );
      } else {
        return this.shownArticles.slice(
          (this.currentPage - 1) * 8,
          this.currentPage * 8
        );
      }
  ...
  methods: {
    handleArticleDeleted(deletedSlug) {
      axios
        .delete(`https://api.realworld.io/api/articles/${deletedSlug}`, {
          headers: {
